...
uint8_t g_data_ready_flag = 0;      // 数据接收完成标志位
volatile uint8_t send_in_progress = 0;  // 发送标志
...
{
    ringbuffer_init(&husart_rx_rb);
    ringbuffer_init(&husart_tx_rb);
    UART_DMA_Init(huart);
}
...
void UART_SendData(uint8_t *data, uint16_t size, UART_HandleTypeDef *huart) 
{
    ...
    for (uint16_t i = 0; i < size; i++) {
        ringbuffer_write(&husart_tx_rb, data[i]);
    }
    ...
    if (!send_in_progress) 
    {
        ...
        // 从环形缓冲区读取数据到 DMA 缓冲区
        while (!ringbuffer_is_empty(&husart_tx_rb) && size_to_send < DMA_TX_BUFFER_SIZE) {
            ringbuffer_read(&husart_tx_rb, &buffer[size_to_send]);
            size_to_send++;
        }
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
    ...
    {
        // 确保接收环形缓冲区没有满
        if (!ringbuffer_is_full(&husart_rx_rb))
         {
            // 将DMA接收到的数据写入接收环形缓冲区
            for (uint16_t i = 0; i < Size; i++)
             {
                if (!ringbuffer_is_full(&husart_rx_rb))
                 {
                    ringbuffer_write(&husart_rx_rb, uart_rx_dma_buffer[i]);
                } else 
                {
                    break;  // 缓冲区满时退出
                }
